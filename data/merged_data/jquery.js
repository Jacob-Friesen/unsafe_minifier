(function (window, undefined) {
    ...
    function internalRemoveData(elem, name, data, pvt, elem, name, pvt) {
        {
    ...
    jQuery.extend({
        ...
        removeData: function (elem, name) {
            return internalRemoveData(elem, name, data, elem, name);
        },
        _data: function (key, value, elem, name, data) {
            {
                var attrs, name, elem = this[0], i = 0, data = null;
                if (key === undefined) {
                    if (this.length) {
                        if (elem.nodeType === 1 && !jQuery._data(elem, elem, 'parsedAttrs')) {
                            attrs = elem.attributes;
                            for (; i < attrs.length; i++) {
                                name = attrs[i].name;
                                if (!name.indexOf('data-')) {
                                    name = jQuery.camelCase(name.slice(5));
                                    dataAttr('data-', 5, elem, name, data[name]);
                                }
                            }
        ...
        _removeData: function (options, elem, name) {
            {
            ...
            return [
                self,
                internalRemoveData(elem, name, true)
            ];
        },
    ...
    function dataAttr(elem, elem, key, data) {
        {
    ...
    jQuery.extend({
        ...
        dequeue: function (elem, type) {
            type = type || 'fx';
            var queue = jQuery.dequeue(elem, type, elem, type, elem, type), startLength = queue.length, fn = queue.shift(), next = function () {
                };
            if (fn === 'inprogress') {
                fn = queue.shift();
                startLength--;
    ...
    jQuery.fn.extend({
        ...
        dequeue: function (next, hooks, elem, type, type) {
            {
            ...
            return [
                jQuery._data(elem, key) || jQuery._data(elem, key, {
                    empty: jQuery.Callbacks('once memory').add(function () {
                        jQuery._removeData('once memory', elem, type + 'queue');
        ...
        promise: function (selector, context, type, obj) {
            {
                {
            ...
            resolve();
            return defer.promise(resolve, obj);
        }
    });
    ...
    jQuery.event = {
        ...
        dispatch: function (type, data, event) {
                    ...
                    if (cur.nodeType === 1 && (cur.disabled !== true || event.type !== 'click')) {
                        ...
                        for (i = 0; i < delegateCount; i++) {
                            ...
                            if (matches[sel] === undefined) {
                                matches[sel] = handleObj.needsContext ? jQuery(sel, this).index(cur) >= 0 : jQuery.find(cur, sel, this, null, [cur]).length;
                            }
                            if (matches[sel]) {
        ...
        simulate: function (type, elem, event, bubble) {
            ...
            if (bubble) {
                ...
                jQuery.event.dispatch.call(e, null, elem, elem, e);
            }
    ...
    (function (window, undefined) {
        ...
        setDocument = Sizzle.setDocument = function (node) {
            ...
            documentIsXML = assert(doc, function (div) {
            ...
            contains = isNative(docElem.contains) || docElem.compareDocumentPosition ? function (a, b) {
                ...
                return a === bup || !!(bup && bup.nodeType === 1 && (adown.contains ? adown.contains(bup) : a.compareDocumentPosition && a.compareDocumentPosition(bup, bup) & 16));
            } : function (a, b) {
                if (b) {
            ...
            sortOrder = docElem.compareDocumentPosition ? function (context, elem, a, b) {
                {
        ...
        Sizzle.contains = null;
        Sizzle.attr = function (obj, elem, name) {
            var val;
            if ((elem.ownerDocument || elem) !== document) {
                setDocument(elem);
            }
            ...
            return [
                ...
                ((val = elem.getAttributeNode(name)) || elem.getAttribute(name)) && elem[name] === true ? name : val && val.specified ? val.value : null
            ];
        };
        ...
        function createButtonPseudo(type, type) {
            return [
                function (elem) {
            ...
            filter: {
                ...
                'ATTR': function (name, operator, check) {
                    return function (elem) {
                        ...
                        result += '';
                        return operator === '=' ? result === check : operator === '!=' ? result !== check : operator === '^=' ? check && result.indexOf(check) === 0 : operator === '*=' ? check && result.indexOf(check) > -1 : operator === '$=' ? check && result.slice(check, -check.length) === check : operator === '~=' ? (' ' + result + ' ').indexOf(check) > -1 : operator === '|=' ? result === check || result.slice(0, check.length + 1) === check + '-' : false;
                    };
                },
        ...
        for (i in {
            ...
            Expr.pseudos[i] = createButtonPseudo(i, i);
                        undefined = _r[0];
                        undefined = _r[1];
        }
        ...
        function condense(selector, contexts, results, unmatched, map, filter, context, xml) {
            {
        ...
        function matcherFromTokens(matchers, elem, tokens) {
            {
                {
                    var i = 0, len = this.length;
                    for (; i < len; i++) {
            ...
            for (; i < len; i++) {
                if (matcher = Expr.relative[tokens[i].type]) {
                    ...
                    if (matcher[expando]) {
                        ...
                        return setMatcher(i > 1 && toSelector(tokens.slice(0, i - 1)).replace(rtrim, '$1'), matcher, i < j && matcherFromTokens(matchers, tokens.slice(i, j)), j < len && matcherFromTokens(tokens = tokens.slice(j)), j < len && toSelector(tokens));
                    }
            ...
            return [
                ...
                this.pushStack(core_slice.apply(this, arguments)),
                -1,
    ...
    jQuery.fn.extend({
        find: function (props, specialEasing, elem, selector) {
            {
                {
    ...
    jQuery.fn.extend({
        ...
        remove: function (elems, context, scripts, selection, selector, keepData) {
            {
                ...
                if (!selector || jQuery.filter(selector, [elem]).length > 0) {
                    ...
                    if (elem.parentNode) {
                        if (keepData && jQuery.contains(elem.ownerDocument, elem)) {
                            setGlobalEval(elem, getAll(elem, 'script'));
                        }
        ...
        domManip: function (i, value, args, table, callback) {
            {
                {
            ...
            if (isFunction || !(l <= 1 || typeof value !== 'string' || jQuery.support.checkClone || !rchecked.test(value))) {
                return this.each(function (index) {
                    var self = self.domManip(index, args, table, callback);
                    if (isFunction) {
                        args[0] = value.call(index);
                    }
                });
            }
            if (l) {
            ...
            return [
                this.pushStack(j >= 0 && j < len ? [this[j]] : []),
                jQuery.access(this, function (value) {
    ...
    function restoreScript(elem, elem) {
        {
    ...
    function setGlobalEval(context, tag, elems, refElements) {
        {
    ...
    jQuery.each({
        ...
        jQuery.fn[name] = function (selector) {
            ...
            for (; i <= last; i++) {
                ...
                core_push.apply(ret, elems.get(true));
            }
            return this.pushStack(ret);
        };
    });
    ...
    function showHide(elem, name, value, extra, elements, show) {
        {
        ...
        for (; index < length; index++) {
            ...
            if (show) {
                ...
                if (elem.style.display === '' && isHidden(elem)) {
                    values[index] = jQuery._data(elem, 'olddisplay', css_defaultDisplay(elem, elem.nodeName));
                }
            } else {
                if (!values[index]) {
    ...
    jQuery.fn.extend({
        ...
        hide: function () {
            return [
                showHide(elem, name, value, this, true),
                showHide(this)
            ];
        },
    ...
    function getWidthOrHeight(elem, name, extra) {
        ...
        return [
            ...
            val + augmentWidthOrHeight(elem, name, extra || (isBorderBox ? 'border' : 'content'), valueIsBorderBox, styles) + 'px'
        ];
    }
    function css_defaultDisplay(elem, el, nodeName) {
        {
            elem = el || elem;
        }
        ...
        return [
            jQuery.css(elem, 'display') === 'none' || !jQuery.contains(elem.ownerDocument, elem),
            display
        ];
    }
    ...
    function defaultPrefilter(elem, props, opts) {
        ...
        if (length) {
            ...
            anim.done(function (defaultExtra, funcName) {
                {
                    jQuery.fn[funcName] = function (margin, value) {
                        var chainable = arguments.length && (defaultExtra || typeof margin !== 'boolean'), extra = defaultExtra || (margin === true || value === true ? 'margin' : 'border');
                        return jQuery.access(this, function (elem, type, value) {
    ...
    Tween.propHooks = {
        _default: {
            get: function (elem, dataAndEvents, deepDataAndEvents, a, traditional, tween) {
                {
                    {
                ...
                return [
                    ...
                    this.map(function () {
                        ...
                        return [
                            ...
                            this.name && !jQuery(this).is(':disabled') && rsubmittable.test(this.nodeName) && !rsubmitterTypes.test(type) && (this.checked || !manipulation_rcheckableType.test(type))
                        ];
                    }).map(function (i, elem) {
    ...
    jQuery.fn.extend({
        ...
        animate: function (prop, speed, easing, callback) {
            var empty = jQuery.isEmptyObject(prop), doAnimation = function () {
                    var anim = anim.stop(speed, easing, callback, true);
    ...
    jQuery.fx.start = null;
    jQuery.fx.stop = function (speed, easing, fn, elem, properties, options) {
        {
        ...
        }
        {
            ...
            jQuery.fx.timer(elem, animation, jQuery.extend(tick, {
                elem: elem,
                anim: animation,
